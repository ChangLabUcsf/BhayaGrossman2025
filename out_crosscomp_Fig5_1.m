...
if ~exist('sent_encoding', 'var')
    load([datapath '/Figure5/Figure5_SentResp.mat'], 'sent_encoding');
end

if ~exist('wordsurp_encoding', 'var')
    load([datapath '/Figure5/Figure5_WordUniVar.mat']);
end
...
cols = getColorsCrossComp(6); % English first, then Spanish
...
for native = types
            ...
            if strcmpi(imgall.(sid).hemi,hemi)
                ...
                if isfield(subject.img_mni, 'elecmatrix')
                    ...
                    if native % spanish for bilinguals
                        % bilingual case and mandarin case (where first column is mandarin)
                    ...
                    else % english for bilinguals
                        % bilingual case and mandarin case
    ...
    % print(fullfile(['brain_images/Fig5_bilingual_' ...
    %          titles{native+1} '.eps']), '-djpeg', '-vector', '-r600')
end
...
ord_anat2 = cellfun(@(x) find(strcmp(x, anat_counts{2}.area)), ...
    anat_counts{1}.area);
anat_counts{2} = anat_counts{2}(ord_anat2, :);
...
figure;
% remove areas with count < 5 and sort by count
% Make this a single bar
anat_counts{1} = anat_counts{1}(anat_counts{1}.count >= 5, :);
anat_counts{1} = sortrows(anat_counts{1}, 'count', 'descend');
ord_anat2 = cellfun(@(x) find(strcmp(x, anat_counts{2}.area)), ...
    anat_counts{1}.area);
anat_counts{2} = anat_counts{2}(ord_anat2, :);
assert(all(arrayfun(@(x) strcmp(anat_counts{1}.area{x}, ...
    anat_counts{2}.area{x}), 1:height(anat_counts{2}))))
b = barh(anat_counts{1}.area, [anat_counts{1}.count ...
    anat_counts{2}.count], 'FaceAlpha', ...
    0.7, 'EdgeColor', 'none', 'BarWidth', 1); % Red color for non-native
...
%% B/C - TRF: Correlating (s)TRF weights across corpora
...
% remove subjects where information not complete
...
disp(['Pearson r(' num2str(length(allsp(:))) '): ' ...
    num2str(corrval) ', p = ' num2str(pval)]);
...
for s = 1:2
    ...
    for ctr = 1:length(els)
        ...
        pval_corrected = 0.01/size(dummy.(SID).resp, 2);
...
fields = {'eng_uv_all', 'sp_uv_all'}; % , '', 'sp_uv_all'
